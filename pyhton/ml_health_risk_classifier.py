...
Supports both command-line interface and Flask API service
"""
...
# Flask imports (only imported when running as API service)
try:
...
def create_flask_app():
    ...
    def health_check():
        ...
        return jsonify({
            'status': 'healthy',
            'service': 'Health Risk Classification API',
            'version': '1.0.0',
    ...
    def predict_health_risk():
        """Main health risk prediction endpoint"""
        try:
            ...
            barangay_data = data.get('barangay_data', {})
            
            if not barangay_data:
                return jsonify({
                    'success': False,
                    'error': 'No barangay data provided'
                }), 400
            
            # Train model with provided data
            training_success = classifier.train_model(barangay_data)
    ...
    def predict_get():
        ...
        return jsonify({
            ...
            'endpoints': {
                'POST /predict': 'Main health risk prediction endpoint',
                'GET /health': 'Health check',
                'GET /predict': 'This help message'
            },
            'example_request': {
                'lookback_days': 14,
                'barangay_data': {
...
def run_flask_server(host='0.0.0.0', port=5001, debug=False):
    ...
    print(f"Starting Health Risk Classification API on http://{host}:{port}")
    print("Endpoints:")
    print(f"  GET  http://{host}:{port}/health - Health check")
    print(f"  POST http://{host}:{port}/predict - Main prediction endpoint")
    print(f"  GET  http://{host}:{port}/predict - API documentation")
...
def main():
    ...
    try:
        # Check if running as Flask server
        if len(sys.argv) > 1 and sys.argv[1] == '--server':
            ...
            run_flask_server(host=host, port=port, debug=debug)
            return
