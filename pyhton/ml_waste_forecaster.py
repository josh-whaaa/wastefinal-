...
def create_flask_app():
    ...
    def health_check():
        ...
        return jsonify({
            'status': 'healthy',
            'service': 'Waste Volume Forecasting API',
            'version': '1.0.0',
    ...
    def forecast_waste():
        ...
        try:
            ...
            training_success = forecaster.train_model(barangay_data)
    ...
    def forecast_get():
        ...
        return jsonify({
            'message': 'Use POST method to send forecasting data',
            'endpoints': {
                'POST /forecast': 'Main forecasting endpoint',
                'GET /health': 'Health check',
                'GET /forecast': 'This help message'
            },
            'example_request': {
                'lookback_days': 14,
                'barangay_data': {
...
def run_flask_server(host='0.0.0.0', port=5000, debug=False):
    ...
    print(f"Starting Waste Volume Forecasting API on http://{host}:{port}")
    print("Endpoints:")
    print(f"  GET  http://{host}:{port}/health - Health check")
    print(f"  POST http://{host}:{port}/forecast - Main forecasting endpoint")
    print(f"  GET  http://{host}:{port}/forecast - API documentation")
